(() => {
  ...
  function queueMobile(){if(window.__householdConsolidatedUiV70)return;clearTimeout(mobileTimer);mobileTimer=setTimeout(()=>{patchMobile();patchForecastDetails()},70)}
  ...
  function boot(){installCss();patchSettings();if(!window.__householdConsolidatedUiV70)queueMobile();const sh=$('semanticCardRowsV48');if(sh){settingsObserver=new MutationObserver(queueSettings);settingsObserver.observe(sh,{childList:true,subtree:false})}if(!window.__householdConsolidatedUiV70){const mh=$('mobileCashflowRowsV60');if(mh){mobileObserver=new MutationObserver(queueMobile);mobileObserver.observe(mh,{childList:true,subtree:false})}const fh=$('cardForecastStableRowsV64');if(fh){forecastObserver=new MutationObserver(queueMobile);forecastObserver.observe(fh,{childList:true,subtree:false})}}window.renderRevolvingCardV66=()=>{patchSettings();if(!window.__householdConsolidatedUiV70){patchMobile();patchForecastDetails()}};window.householdRevolvingCardV66={adjustPlan,revolvingRows,paymentMode,monthlyPayment,balanceValue}}
  function wait(n=0){if($('semanticCardRowsV48')&&(window.__householdConsolidatedUiV70||$('mobileCashflowV60')))return boot();if(n<60)setTimeout(()=>wait(n+1),100)}
