(() => {
  ...
  function accountKey(t){return `${t.source||''}|${t.account||''}`}
  function accountLabel(source,account){const base=SOURCE_LABELS[source]||source||'銀行';if(!account||account==='main'||account==='yucho')return base;return `${base} (${account})`}
  ...
  function legacyRakutenFallback(){
    ...
    return {key:'Rakuten Bank|main',source:'Rakuten Bank',account:'main',label:'楽天銀行',balance:Number(r.latestBalance),asOf:String(r.sourceAsOf),legacy:true,latest:null};
  }
  ...
  function computeLatestBankState(){
    ...
    if(Array.isArray(state.cashTransactions)){
      state.cashTransactions.forEach((t,index)=>{
        ...
        const k=accountKey(t);if(!groups.has(k))groups.set(k,[]);groups.get(k).push({...t,__index:index});
      });
    }
    const accounts=[];
    for(const [key,items] of groups.entries()){
      const latest=latestTransactionForAccount(items);if(!latest)continue;
      accounts.push({key,source:latest.source,account:latest.account||'',label:accountLabel(latest.source,latest.account),balance:Number(latest.balance_after),asOf:latest.date,legacy:false,latest});
    }
    if(!accounts.some(x=>x.source==='Rakuten Bank')){const fallback=legacyRakutenFallback();if(fallback)accounts.push(fallback)}
  ...
  function renderBankBreakdown(latest=computeLatestBankState()){
    ...
    if(!latest){box.innerHTML='<div class="muted">銀行CSVを取り込むと口座別残高を表示します。</div>';return}
    const rows=latest.accounts.map(x=>`<div class="row"><div><b>${x.label}</b><div class="tiny">${x.asOf} 時点${x.legacy?' · 旧取込から復元':''}</div></div><span class="amt">${formatYen(x.balance)}</span></div>`).join('');
  ...
  function repairCurrentCash({persist=true}={}){
    ...
    state.bankAccountBalances=Object.fromEntries(latest.accounts.map(x=>[x.key,{source:x.source,account:x.account,label:x.label,balance:x.balance,asOf:x.asOf,legacy:!!x.legacy}]));
  ...
  if(typeof parseBankMain==='function'){
    const original=parseBankMain;parseBankMain=function(src,file){const result=original(src,file);const latest=repairCurrentCash({persist:false});if(latest){result.balance=latest.cash;result.current_as_of=latest.asOf;result.history_only=!!result.latest&&result.latest<latest.asOf;result.account_balances=latest.accounts.map(x=>({label:x.label,balance:x.balance,asOf:x.asOf}))}return result};
  }
  if(typeof parseYucho==='function'){
    const original=parseYucho;parseYucho=function(src,file){const result=original(src,file);const latest=repairCurrentCash({persist:false});if(latest){result.balance=latest.cash;result.current_as_of=latest.asOf;result.history_only=!!result.latest&&result.latest<latest.asOf;result.account_balances=latest.accounts.map(x=>({label:x.label,balance:x.balance,asOf:x.asOf}))}return result};
  }
