(() => {
  ...
  function openEditor(m){
    ...
    const ps=purchasesFor(st,m),host=$('v75PurchaseRows');host.innerHTML=ps.length?ps.map((p,i)=>`<div class="card" style="padding:9px;margin-top:7px" data-v75-purchase="${esc(purchaseKey(p,i))}"><div class="tiny">${esc(p.source_file||'')}</div><div class="form" style="grid-template-columns:1fr 1fr;gap:7px;margin-top:5px"><div class="field"><label>利用日</label><input type="date" data-v75-pdate value="${esc(p.purchase_date||'')}"></div><div class="field"><label>利用額</label><input type="number" min="0" inputmode="numeric" data-v75-pamount value="${Math.abs(Number(p.original_amount)||0)}"></div><div class="field" style="grid-column:1/-1"><label>利用先</label><input data-v75-pmerchant value="${esc(p.merchant_raw||p.merchant_normalized||'')}"></div></div></div>`).join(''):'<div class="muted">この月に紐づく利用明細はありません。</div>';show()
  }
  ...
  function saveEditor(){
    ...
    document.querySelectorAll('#v75PurchaseRows [data-v75-purchase]').forEach(box=>{const p=findPurchaseByKey(st,box.dataset.v75Purchase);if(!p)return;const d=box.querySelector('[data-v75-pdate]')?.value||'',merchant=box.querySelector('[data-v75-pmerchant]')?.value.trim()||'',a=Number(box.querySelector('[data-v75-pamount]')?.value);if(d)p.purchase_date=d;if(merchant){p.merchant_raw=merchant;p.merchant_normalized=merchant}if(Number.isFinite(a)&&a>=0)p.original_amount=Math.abs(a);p.manual_corrected_at=new Date().toISOString();});
